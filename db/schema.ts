...
// ─── AI Position Assessments (position-manager outputs) ───
export const aiAssessments = pgTable(
  "ai_assessments",
  {
    ...
    aiSource: varchar("ai_source", { length: 10 }).notNull().default("CODE"), // AI | CODE
    reasoning: text("reasoning").notNull(),
    opportunityCostVerdict: varchar("opportunity_cost_verdict", { length: 10 }), // KEEP | REDUCE | EXIT
...
// ─── Position Snapshots (periodic state captures per position) ───
...
// ─── Position Actions Log (audit trail of every action taken) ───
export const positionActions = pgTable(
  "position_actions",
  {
    id: serial("id").primaryKey(),
    positionId: integer("position_id").notNull(),
    ...
    source: varchar("source", { length: 10 }).notNull().default("CODE"), // AI | CODE
    confidence: decimal("confidence", { precision: 5, scale: 4 }),
    reasoning: text("reasoning"),
    result: varchar("result", { length: 10 }).notNull().default("ok"), // ok | failed
    detail: text("detail"),
    executedAt: timestamp("executed_at").defaultNow().notNull(),
  },
  (table) => ({
    positionActionIdx: index("idx_position_actions_position").on(table.positionId, table.executedAt),
  })
);

export type PositionActionLog = typeof positionActions.$inferSelect;
